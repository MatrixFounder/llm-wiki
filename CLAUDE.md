...
`wiki-confirm` / `wiki-alias` / `wiki-merge`, and the Epic 7 RAG layer
`wiki-query`) over an `IndexRepository` DAL (SQLite + FTS5 + WAL).
...
Stub-First green-throughout; 3 VDD gates + `/vdd-multi` ×2 + real-content
dogfood; 599 pytest, mypy strict; **zero schema DDL**, `user_version` stays 4).
No active task at HEAD. See `docs/ARCHITECTURE.md`, `docs/adr/`, and the
archived task/plan pairs under `docs/tasks/` + `docs/plans/`.
...
- `docs/tasks/` + `docs/plans/` — archived task/plan pairs (no
  `docs/TASK.md` at HEAD — most recent: `task-007-wiki-query-rag.md`
  shipped 2026-05-29; per-bead specs at `task-007-01..10-*.md`; review
  records at `docs/reviews/{task,architecture,plan,vdd-multi}-007-*`).
