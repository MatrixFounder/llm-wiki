...
| `wiki-health coverage --vault <vid> [--class C]` | Read-only **coverage** report (R-15 / TASK 036, ADR-006): pages MISSING an expected relation. Three gap `kind`s — `edge` (requirement/capability with no `implemented-by`), `field` (fact with an absent/empty `source:`), and `field-value` (TASK 072: the scalar is PRESENT but a declared non-answer, e.g. `verified_on: "not verified"` — opt-in per rule via `forbid_values`, which no built-in layout ships). Layout-config-driven (`coverage_rules`; cybos ships them); **exits 0 on success** — a gap is data, so it never gates. ⚠️ *On success* is load-bearing: a look-up or argument error keeps the family codes (6 `VAULT_NOT_FOUND`, 2 `INVALID_CLASS`), so a wrapper that ignores `$?` reads a typo'd vault name as a clean bill of health (DF-072-2). Its sibling **lifecycle-drift** (authored `status` vs graph state) rides `wiki-lint` and gates `--strict`. |
