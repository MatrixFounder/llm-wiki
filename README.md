...
| `wiki-health coverage --vault <vid> [--class C]` | Read-only **coverage** report (R-15 / TASK 036, ADR-006): pages MISSING an expected relation. Three gap `kind`s — `edge` (requirement/capability with no `implemented-by`), `field` (fact with an absent/empty `source:`), and `field-value` (TASK 072: the scalar is PRESENT but a declared non-answer, e.g. `verified_on: "not verified"` — opt-in per rule via `forbid_values`, which no built-in layout ships). Layout-config-driven (`coverage_rules`; cybos ships them); **always exits 0** — a gap is data. Its sibling **lifecycle-drift** (authored `status` vs graph state) rides `wiki-lint` and gates `--strict`. |
