...
- [x] **RAG Query Layer (TASK 007)**: `wiki-query` designed as a deterministic `prepare`/`apply` skill (Decision-17, no LLM in Python); query page is a first-class compounding `type=query` artifact; durability secured by the R-6.5e `cites:`→`'cited'` reindex read-side (the §D8 gate, mirroring R-5.3); zero schema DDL; grounding enforced in Python as a triple (`NO_CONTEXT` / `NO_CITATIONS` / `CITATION_NOT_RETRIEVED` — TASK 072 added the floor, without which an empty citations array passed the gate vacuously).
...
- [x] **The CLI I/O contract now says what the CLIs do (TASK 074, 2026-08-10 — DF-072-3/4/5)**: three defects from the TASK 072 exit-code census, all one class — *a universal claim the population was never measured against*. (1) «Every CLI emits **one JSON envelope** + a stable exit code» is FALSE on the usage path: argparse writes usage to **stderr** and exits 2 **before** any `emit()` runs, so stdout is **empty — 22/22 measured**, plus three deliberate non-JSON *success* modes (`wiki-search --format markdown`, `wiki-sync scan --dry-run`, `wiki-config serve`'s stderr banner). The quantifier is now «per **completed subcommand invocation**» at all 19 sites (EN + RU), and the manual's «**branch on `$?`**» advice — which contradicted `wiki-verify-multi`'s own bolded contract — is corrected to *read the envelope, `$?` is a coarse pre-filter*. (2) `wiki-lint --strict` returns **exit 1 carrying a SUCCESS envelope**, a second instance of the divergence the repo had documented as unique to `wiki-verify-multi`'s exit 6, warned about **nowhere** — `skills/wiki-lint/SKILL.md` had no exit table and claimed «Always returns success exit `0`». It now carries a **normative** table (so `test_exit_code_doc_truth.py` holds it to completeness — partition B → A) with the divergence fenced. ★ **Kept at 1 deliberately** (D-074-2): the issue's own proposal to move the gate «to the family's `6`» would land on this CLI's **inherited** `INVALID_INDEX_DB`, *reproducing* the exit-6 ambiguity rather than removing it — and exit-1-on-findings is the universal linter convention. The collision is instead made **discriminable and pinned**: a crash emits **no envelope** (empty stdout), a tripped gate emits a parseable one with no `error` key. (3) `wiki-init` emitted a **`received`** key echoing the offending `vault_id` — the one key `components.md:291` names forbidden (CWE-117) — at three sites for 14 months, because the three runtime canary suites enumerate their CLIs **by hand** from the instances already known, and `wiki_init` was in none of them. Now `{error, field, reason, pattern}`. ★ **The real fix is the population**: `tests/test_cli_envelope_contract.py` derives BOTH new gates from a runtime `bin/` walk (reusing `_discover_clis()`), so a CLI added tomorrow is in scope without editing the file. ★★ **And a 3-critic `/vdd-multi` pass then caught the fix committing this task's own thesis three times** — corrected in the same task, which is the part worth remembering: (a) `CLAUDE.md`/`AGENTS.md`/both manuals claimed «**both** halves are gated» while the gate's own CANNOT block disclaims the three non-JSON modes; (b) «the blind spot cannot grow by omission» was **false** — the pin measured CLIs with *zero* literal envelopes, a degenerate case, while **41 emit sites are invisible and 10 sit in CLIs the pin counted as covered** (now pinned as a per-CLI *count*); (c) the scan claimed «every source file of every CLI» while `_common.py` — which emits the `INVALID_INDEX_DB` envelope **inherited by all 19 wiki CLIs** — sat outside all 22 scan sets (now a computed set-difference gate). The declared blind spot **was hiding a live defect**: `wiki-reindex` emitted `str(e)` from a bare `except Exception` into `skipped[]` (vault paths + frontmatter line/column; *not* literal source bytes — the reviewer overstated that, and the correction is recorded) → now `type(e).__name__`. Four further live defects fixed en route (`wiki_init` still echoing `str(args.vault)` under a non-denylisted key; a `TypeError` on any non-str YAML `vault_id`; a missing `FileNotFoundError` guard in `reconcile`; "has no vault_id" reported for *unparseable* YAML), three out-of-diff findings **filed rather than folded in** (DF-074-1/2/3), and three transcribed counts corrected in a task about not transcribing counts. **Declaring a blind spot is not closing it; counting it is the weaker but honest guarantee, and it is what ships.** 47 assertions + 5 visible skips, non-vacuity + mutation controls on each; **3216 passed**; `mypy --strict` clean; **zero DDL**, **no exit code changed**.
