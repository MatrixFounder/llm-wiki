...
- **Q-013-b (TASK 013): DAL shape** — **RESOLVED: extend `search_pages` with optional `where_fields: list[tuple[str,str]] | None` and make the FTS `query` optional.** When a MATCH term is present → today's `pages_fts JOIN pages` path + `AND json_extract(p.frontmatter_json, ?) = ?` predicates; when the query is empty AND ≥1 `where_fields` → a **non-FTS path** (`SELECT … FROM pages p WHERE <predicates>`, no `pages_fts` join, no BM25). One method, internal branch — no separate `filter_pages()`.
- **Q-013-c (TASK 013): query-less ordering** — **RESOLVED: `ORDER BY p.project, p.slug` (deterministic).** No BM25 exists without a MATCH term; lexical `(project, slug)` is stable and test-lockable.
- **Q-013-d (TASK 013): field-name injection policy** — **RESOLVED: static allowlist regex `^[a-z][a-z0-9_]*$` at the CLI boundary AND re-validated in the DAL (library-caller defense, per the skill-family convention); the JSON path `'$.'+field` and the value are BOTH bound parameters** (SQLite accepts `json_extract(col, ?)`). A per-layout `filterable_fields` allowlist is a YAGNI follow-up. Error envelope `INVALID_FILTER` (exit 2) never echoes the value (CWE-209/117).
