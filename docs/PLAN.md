...
- **012-12** R-X3 dogfood: run the splitter on this repo → `docs/issues/*.md`; re-render
  ...
  --types known-issue` returns one issue.
...
- ✅ KNOWN_ISSUES: delete + `--auto-indexes` byte-identical (modulo GENERATED-AT); `wiki-lint` flags a manual edit; `wiki-search "hash drift" --types known-issue` returns one issue.
