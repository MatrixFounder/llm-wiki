...
- [x] `config/sync-config.schema.yaml`: additive `x-wiki-scope` (root-only|cascading) on
...
- [x] `scripts/wiki_index/sync_config.py`: additive `SyncConfigError.reason` machine field
...
- [x] New package `scripts/wiki_skills/wiki_config/`: `_uimodel.py` (schema → UI-model
      ...
      `--report` md sidecar).
- [x] `bin/wiki-config` wrapper (clone `bin/wiki-query` no-cd pattern).
- [x] Tests: `test_wiki_config_provenance.py` (equivalence release-gate vs
...
## Phase 2 — validate
- [x] `_findings.py`: `ConfigFinding` + taxonomy registry (33 codes → severity, tier,
      value-free message template) + renderers (histogram / md report / json sidecar).
- [x] `_lint.py`: pass 0 discovery walk; pass 1 hard gates via loaders (catch + enumerate
      schema errors via `iter_errors`); pass 2 per-file advisory; pass 3 cross-level.
      All three config systems.
- [x] CLI `validate [<folder>] [--strict] [--json-sidecar <p>] [--report <p>]`; exit 6 on
      error-severity, `--strict` promotes warnings.
- [x] Tests: `test_wiki_config_validate.py` — one fixture per code; golden run over samples/;
      loader↔linter reason lockstep; CWE-209 grep-asserts.

## Phase 3 — write core (doctor/fix/restore/set)
- [x] `requirements.txt` + `ruamel.yaml>=0.18`; mypy wiring confined to `_edit.py`.
- [x] `_edit.py`: the sandwich — hardened gate BEFORE (our `_NoAliasSafeLoader` + schema +
...
- [x] `_backups.py`: backup/list/prune(10)/restore; `atomic_write_text` (+ optional
      `suffix` param in `_common.py`).
- [x] `_doctor.py`: FixPlan; finding→fix mapping (tiers per taxonomy); TOCTOU sha256
      pinning; PARSE_ERROR recovery (restore-from-backup | archive-`.broken`+scaffold).
- [x] CLI `doctor`, `fix [--from-plan] [--yes] [--dry-run] [--no-backup]`,
      `restore [--list|--to] [--yes]`, `set`/`unset <folder> <pointer> [<value>]`
      (root-only-in-subfolder → exit 2 with explanation).
- [x] Tests: `test_wiki_config_doctor.py` — fix round-trip per code, idempotency,
      adversarial downgrade, TOCTOU exit 2, backup retention/restore reversibility.

## Phase 4 — templates
- [x] `templates/sync-profiles/{meeting-zone,lessons-mirror,connector-zone,article-zone,
...
- [x] `_templates.py`: registry (builtin + `<vault>/.wiki/templates/`, builtin wins on
...
- [x] CLI `init`, `templates`; `TEMPLATE_DRIFT` finding into `_lint.py`.
- [x] `config/sync-config.schema.json` projection + identity test; `# yaml-language-server:`
      modeline injected by init; `SCHEMA_MODELINE_MISSING/STALE` findings.
- [x] Tests: `test_wiki_config_init.py` per TASK acceptance (level mismatch exit 2,
      byte-identical re-init, ReDoS var exit 6 without echo, all builtins pass gates).

## Phase 5 — HTML report
- [x] `_report.py`: pure `render_html(model) -> str` (string.Template + html.escape all;
...
- [x] CLI `report [--out] [--open] [--all-folders] [--md <path>]`; default out
      `<vault>/.wiki/config-report.html`; `webbrowser.open`.
- [x] Tests: `test_wiki_config_report.py` — snapshot fixtures (Cyrillic/space/emoji names,
      ignored-root-key, invalid-file, icloud-placeholder, 3-level cascade), determinism.

## Phase 6 — serve (web editor)
- [x] `_server.py`: stdlib `http.server`, single-threaded; 127.0.0.1:ephemeral; token
...
- [x] Inline app (ONE html string module): schema-driven form renderer (enum dropdown,
...
- [x] Tests: `test_wiki_config_serve.py` — API contract via http.client against an
...
## Phase 7 — delivery
- [x] `skills/wiki-config/SKILL.md` (disambiguation vs `config/wiki-config.schema.yaml`),
      `commands/wiki-config.md`; re-run `bin/install-globally.sh`.
- [x] `docs/manuals/obsidian-llm-wiki_manual.md` section (+ exit-code table row,
      connector-zone path fix); `README.md` (18 CLIs); `docs/ARCHITECTURE.md` +
      `docs/architectures/` section; `docs/ROADMAP.md` entry.
- [x] Dogfood: report+serve on `samples/Demand-generation` and
      `samples/personal-vault-dogfood` per TASK §6 acceptance; fix any findings.
- [x] Final gates: full pytest, mypy --strict, golden samples run; merge to main.

---

## Completion notes (2026-07-11)

All 7 phases shipped on `task-058-wiki-config-interface`; 2210+ tests, mypy
--strict green. Deviations from the letter of the plan (spirit preserved):

- `templates/connector-zone.sync.yaml` was NOT moved (12 documents + a test
  reference it); `templates/sync-profiles/connector-zone.yaml` is the registry
  copy with the header.
- SCHEMA_MODELINE_* findings fire only on files carrying the template stamp —
  hand-authored files are never nagged.
- Dogfood regression fixed: a RELATIVE `--vault-root` now resolves (found by
  running `show` on `samples/Demand-generation`, not by tests).
- Dogfood results: Demand-generation → 3 true REDUNDANT_OVERRIDE infos
  (Lessons repeats inherited mirror keys); personal-vault-dogfood → clean;
  Lessons provenance matches the acceptance picture exactly (`group_key`
  HERE/shadows-root; `source_state`, `provenance_ref`, `mode` ↑ root).
