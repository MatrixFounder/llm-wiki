...
### R-24. Живой корпус не несёт референтов — координаты `path:line` протухают молча

`agentic-development` v3.27.0/v3.28.0 добавил в `documentation-standards` **референт**: ссылка
`path:line` несёт рядом код-спан с именем символа или дословной подстрокой цитируемой строки, и
резолвер проверяет её. Механизм **уже доступен здесь** через симлинк
`.agent/skills/documentation-standards` — писать ничего не надо:

```bash
python3 .agent/skills/documentation-standards/scripts/check_positional_refs.py \
  --all docs/ARCHITECTURE.md docs/TASK.md docs/PLAN.md docs/architectures
```

**Замер на сегодня:** `0 with a referent, 82 without (not examined), 15 unresolvable, 1 pinned` —
98 координат в живом корпусе, из них **ни одна** не проверяема, а 15 не резолвятся вовсе.

**Почему это не косметика.** Ссылка на номер строки — утверждение о состоянии, записанное в
грамматике адреса, и её ложность не наблюдаема ничем, кроме чтения. В соседнем проекте замерено:
из 142 разрешимых координат живого корпуса **26 указывали не туда**, и ни об одной резолвер не
сообщал — каждая была исправна по всем осям, которые он тогда проверял. Причина структурная:
документные фазы пишут координаты до того, как фаза разработки правит цитируемые файлы.

**Апгрейд ничего не сломал и ничего не требует.** Координата без референта отмечается как
*не проверена*, а не как дефект — гейт зелёный. Четыре ревью-чек-листа (тоже симлинками) уже
требуют прогонять резолвер и цитировать строку охвата, и прямо запрещают ревьюеру требовать
референты там, где их нет. Так что это работа по желанию, а не условие.

**Объём.** 15 нерезолвимых — путь от корня вместо голого имени. 82 без референта — простановка,
но **только после аудита дрейфа**: заякорить раньше значит припечатать те координаты, что уже
указывают не туда, и сделать их формально верифицированными.

**Готово, когда** прогон выше даёт `0 unresolvable`, ноль `REFERENT_ABSENT`/`REFERENT_AMBIGUOUS`,
и доля `with a referent` в строке охвата — та, которую владелец счёл достаточной. Строка охвата
сама метрика прогресса: её можно цитировать в ревью, не дожидаясь полного покрытия.
