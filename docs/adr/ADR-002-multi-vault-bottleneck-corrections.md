...
>
> **Amendment (TASK 012, NO schema change — 2026-06-01).** R-X1 universalises the
> layout engine (config-driven) and R-X3 migrates `docs/KNOWN_ISSUES.md` from a
> single hand-edited ledger to **per-file Class-A issues + an auto-rendered
> Class-B ledger**. This registers a **new Class-B sub-case: "rebuildable
> markdown"** — a derived markdown *file* (not a DB column/table) regenerated
> from Class-A *markdown* sources:
> - **Before:** `docs/KNOWN_ISSUES.md` was **Class A canonical** (the authoritative
>   issue ledger, hand-edited).
> - **After:** `docs/issues/<id>-<slug>.md` are **Class A canonical** (per-issue
>   status / severity / history / body); `docs/KNOWN_ISSUES.md` becomes
>   **Class B — rebuildable markdown** (an aggregated projection regenerated by
>   `wiki-index-render --auto-indexes` from the per-issue files, grouped/sorted
>   per the layout's `auto_indexes[]` config — PW-H).
> - **Distinction from prior Class B:** existing Class-B artifacts are either DB
>   columns/tables/FTS rebuilt from markdown, or the `index.md`/`log.md` rendered
>   views (Class-B canonical-on-DB, re-rendered from DB *views* — see "Special
>   cases" below). The KNOWN_ISSUES ledger is the **first Class-B artifact that is
>   markdown rebuilt from other markdown** (Class-A per-issue files), via the
>   `auto_indexes[]` renderer, **not** a SQL view.
> - **Rebuildability invariant (the §D8 test, in render form):** deleting
>   `docs/KNOWN_ISSUES.md` and running `wiki-index-render --auto-indexes`
>   reproduces it **byte-identically modulo a `<!-- GENERATED-AT: <iso8601> -->`
...
>   native-enum extension, stays deferred.)
> - **Layout grammar classification:** the **built-in** `layouts/*.yaml` ship with
>   the engine and are **Class C operational** (code/config bundled with the tool);
>   an operator-authored `<vault>/.wiki/layout.yaml` (or `WIKI_SCHEMA.md`
>   `layout_config:`) override is **Class A** (a git-tracked vault file read on
>   reindex). Per-vault config identity (`config_loader.py` /
>   `wiki-config.schema.yaml`) is unchanged.
> - **Karpathy byte-identity (global no-regression invariant):** the built-in
>   `karpathy.yaml` is a *validated projection* of `layout.py`; a Karpathy vault
>   re-indexes to byte-identical rows (the standing §D8 rebuildability test, now
>   additionally guarded by a golden-snapshot harness +
>   `test_karpathy_config_matches_layout_constants`). `flat`/`per-project` resolve
>   to `karpathy.yaml` (the `layout:` field never gated the walk).
