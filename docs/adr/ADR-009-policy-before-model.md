...
| 2. Semantic objects + typed links over heterogeneous sources | **Substantial; two gaps** | Entities (8-type enum) + hard-unique aliases + 13 typed page classes (ADR-003) + 15 inverse-closed edge kinds (ADR-004) + graph-derived `--as-of` temporality (TASK 034). Gaps: no declared edge domain/range or property enums (→ R-19); sources are one-shot file imports, no refresh loop (→ R-18) |
