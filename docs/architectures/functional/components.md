...
⚠️ **Read the sentence above as the DENYLIST it enforces, not the allowlist it also reads like.** Its "`{error, field?, reason, violations?}` **only**" half is false in practice and always has been: live error envelopes legitimately carry `hint`, `pattern`, `expected_path`, `wiki_schema_path`, `unknown`/`known`, `integrity`, `message`/`details`. The enforced and enforceable property is: **no `content`/`value`/`raw`/`received` key, and never a byte of source-body content; other diagnostic keys are permitted provided they carry no operator- or source-supplied value.** (Flagged during TASK 074's `/vdd-multi` pass — shipping a *strengthened* citation of a false universal, inside the task that exists because a universal was never measured, is how the next census re-files it.)

⚠️ **The invariant held; its POPULATION did not** (DF-072-5, TASK 074). The runtime canary suites enumerate their CLIs **by hand**, and the hand-list was derived from the CLIs the invariant was written for — so `wiki-init`, which violated it at three sites with a literal `received` key, was the one CLI the canary could never fire on. It went unseen for 14 months; a one-token grep would have found it at any point. The fix is not another canary but a **population-derived** one: `tests/test_cli_envelope_contract.py` walks `bin/` at runtime (reusing `_discover_clis()` from `tests/test_exit_code_doc_truth.py`), AST-scans every error-bearing `*emit(...)` dict-literal payload — recursively, and error-bearing **at any depth**, mirroring the TASK 064 runtime walk — and fails on any forbidden key. It covers every CLI's own sources **plus** a computed shared-emitter population (the set difference: every `emit`-calling file under `scripts/` no CLI scan reaches). That second half exists because the first cut missed `_common.py`, which emits the `INVALID_INDEX_DB` envelope **inherited by every `wiki-*` CLI** and sat outside all 22 per-CLI scan sets.

Automatic inclusion, explicit exclusion — and the exclusions are **counted, not merely declared**. `_NO_LITERAL_ERROR_ENVELOPE` names the CLIs with *nothing to check*; `_NON_LITERAL_EMIT_SITES` pins the per-CLI **count of emit sites the static scan cannot see** (41 at time of writing, 10 of them in CLIs the first cut counted as fully covered). The first pin alone caught only a CLI falling to zero literals, so "the blind spot cannot grow by omission" was false as first written — DF-072-5's own failure mode, reproduced one level up inside its fix, and found by an adversarial pass rather than by the mechanism. ★ It was hiding a real defect: `wiki-reindex` was emitting `str(e)` from a bare `except Exception` into `skipped[]`, carrying vault paths and frontmatter line/column coordinates (fixed to `type(e).__name__`).

**What this gate does NOT prove:** it checks four **key names**, not values — `{"error": …, "offending": v}` passes it. The hand-listed runtime canaries remain and are not redundant: they check *values* at runtime on the CLIs they cover, this checks *keys* over the complete roster. Neither alone is "no envelope echoes the offending value."
...
- `apply(args) → int` — load+bound verdict JSON; re-read query page + recompute `answer_hash` vs `--answer-hash` (`ANSWER_CHANGED`); **re-check both grounding floors** (`NO_SOURCES` on an empty `cites:` or an empty examined set — DF-072-1: `--verify-hash` is optional, so `apply` runs without the `prepare` that carried them, and a floor only upstream let a `verdict: pass` be filed + self-indexed over zero sources at exit 0); validate verdict enum + finding sources ⊆ examined set; `_sanitize_markdown_text` the verdict body (egress, reused from `_common`); atomic-write `_verifications/<verification_slug>.md` (`O_NOFOLLOW` + tempfile + content-hash skip, `--force` overrides); `upsert_page` + `replace_refs(verifies [+ cited])` on one connection; `record_verify_state`; append a `verify` `log_event`. Exit 0 (PASS or `--fail-on=none`) / **6 (VERDICT_FAIL)** / 1 / 2 / 4.
...
| 2 | `NO_SOURCES` | the query page's `cites:` is empty, **or** every cite is unresolvable so the examined set is empty (nothing to verify against — R-8.8a). Both floors hold in **both** subcommands (DF-072-1); the `reason` string distinguishes them. |
