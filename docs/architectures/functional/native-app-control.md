...
**The decision: a small local Obsidian plugin, `agent-bridge`, not `eval`.**
*(This said "~110-line" until TASK 070. It was never true: `main.ts` was already 248 lines at TASK
068's ship commit, and :276-277 below contradicted the figure in the same document. As shipped today:
`main.js` **305** lines — the only file Obsidian executes, hence the actual surface — built from a
**457**-line `main.ts`. The argument for the plugin over `eval` never rested on the exact count, but a
size claim in a security context is a claim about reviewability, so it is measured, not estimated.)* Two plain-
...
- **Nothing runs `tsc` automatically** — no CI, no pytest, no script; it is a hand-typed `npx tsc
  --noEmit` that no gate will ever repeat. *(**Fixed in TASK 070**, and called out separately because
  the OQ2 blockquote below does NOT rescue this bullet — it speaks to the other two. `tsc --noEmit`
  now runs **in pytest**: `tests/test_agent_bridge_build_drift.py::test_main_ts_typechecks_clean_
  against_the_real_obsidian_types`, gated on `_require_tsc()` alone — never on esbuild, or a missing
  bundler would silently skip the typecheck. It also runs inside `build_agent_bridge.py --write`,
  which refuses to rebuild or re-pin on any error.)*
...
> **OQ2 — CLOSED (TASK 070, 2026-07-16).** All three landed: the vendored `obsidian.d.ts` is deleted
> and `main.ts` type-checks against the real `obsidian@1.12.3` (exact-pinned, `== minAppVersion`);
> `main.js` is generated by `scripts/build_agent_bridge.py --write`, whose `--write` runs `tsc
> --noEmit` first and **refuses to re-pin on a type error**; and `tests/test_agent_bridge_build_drift.py`
> hash-pins **both** files with a byte-identity L1 on top. Wiring the real types made the fabrication
> visible immediately — **exactly 3 errors**, all the invented `getMode`. **H2 is unchanged and stays
> open**: no JS test runtime exists, so nothing in the suite executes plugin logic. What closed is the
> *derivation* gap (`main.js` is no longer remembered) and the *contact* gap (`tsc` now checks
> Obsidian's own typings) — **not** the behavioural one. The live dogfood is still the only gate that
> can catch a runtime fault, which is why TASK 070 ships one.
...
| Item | Status |
|---|---|
| Popout windows get no persist highlight | **Real → FIXED in TASK 070 (R-070-6); see §2.2.3.** `main.ts` appended the `<style>` to the bare `document.head` (main window); a popout owns its own `document`. Shipped fix: `EditorView.baseTheme` rides in the extension array, so CM6 mounts the rule into each view's own root. Source-shape tests pin it (`tests/test_popout_selection_highlight.py`); **the render itself is dogfood-verified, not test-verified** — `var(--text-selection)` resolving inside a popout document is runtime behaviour and is not assertable from source. ⚠️ **This row's earlier "the fix needs an `on("window-open", …)` overload" is WRONG and is kept struck-through rather than deleted, because it did real damage:** it was reasoned from the vendored fiction, whose `EditorView` declared only `hasFocus` + `state` — **`baseTheme` was absent**. TASK 070's first draft duly prescribed ~30 lines of `window-open` plumbing (its C-2) by reading this row as a decision. The real fix is **one line**: `EditorView.baseTheme`, letting CM6 mount the CSS per view root (dedup structural, cleanup free). A ledger entry written from a fiction propagates the fiction — which is the whole thesis of §2.2.3. |
...
**TASK 070 collapses all three into one real dependency, and `main.js` becomes a build product.**
`main.ts` + the real pinned `obsidian` package are canonical; the drift gate is `main.js`'s
rebuildability check — **the same architectural ROLE** `wiki-reindex --full` plays for the index.
Nothing is "kept in lockstep by hand" any more, because nothing *can* be.

> ⚠️ **That is an analogy about rebuildability, NOT a class assignment — and the distinction is
> load-bearing.** This sentence read *"demotes `main.js` to Class B"* until the TASK 070 adversarial
> pass, i.e. it asserted, 19 lines below the callout above that refutes Class B point-by-point,
> exactly the frame that callout was written to retract. `main.js` is **Class C** (repo-bundled
> operational code); ADR-002 §D8's classes are vault-scoped, and `wiki-reindex --full` neither
> rebuilds `main.js` nor knows it exists. ★ Three independent review lenses flagged this one line,
> and the callout above had already named the mechanism in advance: *"an appealing-but-unratified
> frame in a living doc gets cited as settled by the next reader."* The next reader was this
> document's own summary sentence, and the frame survived precisely because it was the quotable one.
...
- **L1 — byte compare, needs `esbuild` (NOT node).** Rebuilds and compares. Catches exactly L0's
  blind spot. Each tool carries its own presence check: esbuild's postinstall installs a
  platform-native binary and runs without node, while the separate `tsc` gate needs node. One
  shared `toolchain_present()` would skip the **typecheck** whenever esbuild is absent — a green
  meaning "not checked" — which is the R-070-9(a) hazard, and is the shape the design scratchpad
  shipped as copy-paste-ready code.
...
`minAppVersion` enforcement is documented but **unobserved** (OQ-070-3);

> **OQ-070-4 — CLOSED by observation, 2026-07-16.** `instanceof` across window realms **holds**. A
> popout read returned `{"ok": true, …, "source": "active"}`: `source: "active"` proves `activeEditor`
> was non-null, so resolution took the `if (active)` branch and ran `!(active instanceof MarkdownView)`
> against a view whose DOM lives in the **popout's** realm — and it passed. The supporting reason (real
> `obsidian.d.ts:62` declares `instanceOf<T>()` on **`Node`**, documented as *"a drop-in replacement for
> instanceof checks on **DOM Nodes**"* — the per-realm hazard belongs to **DOM** classes, whereas
> `MarkdownView` comes from Obsidian's single app bundle) was correct, but it is recorded as an
> explanation of an observed fact, never as its substitute.
>
> **The diagonal — ALSO CLOSED, same day, and this is the one that mattered.** `recent-editor` *from a
> popout* (`activeEditor` null ⇒ `isAttached()` ⇒ does `getLeavesOfType("markdown")` include popout
> leaves?) is the scenario the plugin exists for, and neither earlier run touched it — two half-covering
> runs do not compose into it. **`getLeavesOfType("markdown") DOES include popout leaves.`** Proven with
> a note that existed **only** in a popout, carrying a unique sentinel: from the integrated terminal,
> `source = recent-editor | POPOUT-PROBE-7f3a91c2…`. `recent-editor` proves the `else` branch ran; the
> sentinel proves the resolved leaf was the popout's, because that string exists nowhere else.
>
> ★ **Method note worth keeping.** The attempt before it returned `recent-editor` with correct text and
> *looked* like a pass — but the envelope cannot distinguish "resolved the popout's view" from "resolved
> a main-window view", so the claim rested on the operator's recollection of window state. A sentinel
> that can only come from one place takes the human out of the evidence chain. Same shape as the
