...
- Computed from an `http(s)://` URL under one of **`policy.EXTERNAL_PROVENANCE_KEYS`** (TASK 061 / R-061-3 — the ONE source of truth, deliberately not re-listed here; both halves of the Q-050-3 contract are *rendered* from it), the `_raw/` path segment, and inbound `verifies` refs (ONE batched `find_verified_slugs` per prepare and per edge-depth-level — no N+1); it supersedes the synthesis contract's `_raw/` path heuristic with machine-readable signal.
- **The constant carries the VALUE SHAPES, not just the keys** (061 VDD fix-loop / H2): a provenance key's URL counts as a **scalar**, inside a **list**, or inside a **list of `{…, url: …}` objects** — the shape our own `generate-detailed-meeting-summary` emits and our own `all_cited_sources` reads. Enumerating the keys while *assuming* a scalar value is the same fail-open one level down, and it was live: **17 pages** derived `internal` and passed `--min-trust internal` (both halves agreed — *alignment is not the security property; fail-closed is*). The walk is **bounded at 3 levels** (no `json_tree` recursion on the hot path) and the uncovered shapes are test-pinned, so the limit is visible rather than merely true.
- **SQL half = ONE `json_each` pass** over `frontmatter_json` (061 VDD fix-loop / M2) — the blob is parsed **once per row for all keys**, short-circuiting on first match, with cost **flat in the key count**. It was one `json_extract` per key × scheme = **12 blob re-parses per candidate row** (6 at TASK 050; silently doubled when 061-06 grew the key list — an "accepted" cost that grew with an edit nobody re-measured). SQLite does no CSE on row-dependent calls, and the predicate rides the *metadata* query shape, which has **no index for its ordering**: the vault partition is scanned and sorted in a temp b-tree, so the `LIMIT` does **not** bound predicate evaluation. Fixed by query **shape**, not by an index (**P-5 holds**; zero DDL).
- **TASK 061 / R-061-7 — the frontmatter key is the OPERATIVE signal; `_raw/` is a BACKSTOP.** The `_raw/` limb is correct but **unreachable through the normal index path**: all 4 built-in layout grammars (`karpathy` — also `flat`/`per-project`, `dev-project`, `obsidian-personal`, `cybos`) carry `**/_raw/**` in `ignore`, so a `_raw/` capture is never indexed and can never *be* a hit. Live-vault confirmation: **0 of 3267 pages are external-by-path; 100% of the `external` tier is frontmatter-URL-derived.** The limb survives for direct `wiki-index-upsert` calls and custom layouts that choose to index captures — do **not** delete it, and do **not** describe it as a retrieval-time signal.
