...
- ~~**API rate-limits** (`wiki-source-light` → Anthropic): exponential backoff (1s, 2s, 4s) up to 3 retries.~~ ⚠️ **NO SUCH MECHANISM (corrected 2026-08-06, TASK 072)** — `wiki-source-light` was never shipped, and under Decision-17 `scripts/` makes no LLM-provider call. No backoff exists on this path. Retry behaviour for the external `html` / `pdf` subprocesses belongs to those skills, not to this tree.
