...
- **In transit**: ⚠️ **Corrected 2026-08-06 (TASK 072).** The previous wording — "HTTPS для всех external API calls (**Anthropic**)" — is stale. Under **Decision-17** `scripts/` makes **no LLM-provider call at all**: `grep -rnE '^\s*(import anthropic|from anthropic)' scripts/` → **0**, gated over the whole population (`tests/test_extract_decisions_dispatch.py`). The real in-transit surface is the **external subprocesses** (`html` / `pdf` / `transcript-fetcher`), which reach the network themselves, plus the two direct egress sites in `_fetch.py` — see A10 below.
- **PII**: ⚠️ **Corrected 2026-08-06 (TASK 072).** The previous wording claimed `wiki.research.private_concepts` + `private_tags: [confidential]` had a schema ready. **That is FALSE**: `grep -rn 'private_concepts\|private_tags\|wiki\.research' config/` → **0 hits across all 7 files**. No such schema exists, or ever did. The shipped surface is `classification:` + `--audience` (**ADR-009 / TASK 049**) — an SQL predicate applied BEFORE content reaches a model context. ⚠️ And it is **measured to fire on 0 pages** in every live vault: the mechanism is **AVAILABLE, not IN USE**. Recorded that way rather than as "ready", because "ready" reads as "working".
...
- **A10 SSRF**: ⚠️ **REWRITTEN 2026-08-06 (TASK 072) — the previous text was false twice over.** It named `wiki-source-light`, a **never-shipped** surface (`scripts/wiki_source/` holds only `__init__/base/manual/parsing.py`; **no** `bin/` launcher; the sole reference is a Phase-3b *plan* docstring at `base.py:8`), and asserted it "does not accept a user-supplied URL".

  **The egress surface is EXACTLY TWO call sites**, both in `scripts/wiki_skills/wiki_import_article/_fetch.py`: `_download_pdf` and `_download_raw_html`. Enumerated, not "mostly".

  **★ THE CONTROL IS BUILT (2026-08-06, bead 072-07).** Neither site calls `urlopen` any more — both go through the external `html` skill's `get` verb, i.e. through that skill's SSRF-guarded ladder: **resolve → pin → assert-public → bounded read, on EVERY hop**. Both `S310 (operator URL)` suppressions were deleted along with the calls they excused.

  - **Why the old justification had to go.** (1) `/wiki-reload` re-fetches a URL taken out of a note's **own frontmatter** — that is H-6 **data**, not operator input. (2) urllib follows 30x silently, so **every hop after hop 0 is attacker-chosen** regardless of who typed hop 0.
  - **Structural invariant**: `tests/test_import_ssrf.py::test_fetch_module_makes_no_bare_urlopen_call` — by **AST**, not grep (a grep-based guard would match its own prose), plus a mutation control proving the scanner can fire.
  - **Fails CLOSED**: if the installed skill lacks the verb (`--help` does not list `html get URL`), the import stops with `DEPENDENCY_MISSING` naming three remediations. There is **no fallback to `urlopen`** — a fallback would re-open the hole precisely when the guard is absent.
  - **Branch on `details.kind`, never on the bare exit code**: every `FetchFailed` maps to 10, so a security refusal, a deadline and a transport error are otherwise indistinguishable. That exact gap once made the skill's **own** refusal tests vacuous — with the blocklist disabled they still passed, having egressed to RFC-1918 and received a real `HTTP 404`.
  - **Proved by EXECUTION, offline**: five hostile classes (loopback · RFC-1918 · link-local metadata `169.254.169.254` · IPv6 loopback · `file://` scheme) refused through **both** real call sites, **plus a redirect to a private host** — the case the old justification could not survive. And, non-negotiably, a **positive control**: the same URL through the same code path, permitted via `HTML_SSRF_ALLOW_NETS`, downloads successfully. Without it, the whole refusal set would be equally consistent with a guard that refuses everything.

  ⚠️ **Two residual risks, stated rather than hidden.** (1) Address pinning re-opens whenever `HTML_PROXY` is set: the proxy resolves the target itself. (2) Under a **fake-IP resolver** (this machine runs one — real hosts resolve into `198.18.x.x`, which is why `HTML_SSRF_ALLOW_NETS=198.18.0.0/15` is the skill's shipped default) the pin binds the **synthetic** address, and the synthetic→real mapping belongs to the proxy tool. Both are documented in the skill's own SKILL.md §5. **Do not claim DNS rebinding is universally closed.**
