...
| R-6.7 (grounding / no-hit refusal) | §2.1 RAG Query Layer Operational invariants ("grounding enforced in Python"); CLI surface `--min-hits` + the grounding TRIPLE `NO_CONTEXT` (nothing retrieved) / `NO_CITATIONS` (grounding not claimed) / `CITATION_NOT_RETRIEVED` (grounding claimed outside the hit set) | UC-18 AC (no synthesis on empty retrieval); UC-21 AC (citation ∉ hit set refused at boundary, `project/slug` key); TASK 072 `test_empty_citations_refused` + `test_min_hits_zero_cannot_file_an_ungrounded_page` (both executed-RED, envelope-asserting). ⚠️ UC-18's alternate flow A1 ("`--min-hits 0`: operator explicitly permits a no-context answer") is FALSIFIED as of TASK 072 — that path can no longer file a page |
