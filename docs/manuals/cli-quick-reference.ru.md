...
layout), два двухпроходных экстрактора — `/wiki-extract-concepts` (уплотнить
проиндексированный источник до страниц `_concepts/`) и `/wiki-extract-decisions`
(типизированное знание — страницы решение/требование/риск + рёбра, на layout-ах, которые
их отображают) — и `/wiki-reload` (*«перезагрузи заметку»* — обновить битый/устаревший
**веб-клип на месте** по URL из его фронтматтера: reader-mode fetch, зачистка site-chrome,
тот же файл, фронтматтер сохранён, переиндексация; это НЕ `/wiki-import`, который создаёт
НОВУЮ суммаризированную заметку). Агент держит вас в курсе всего, что пишет
