...
- Для `wiki-query` / `wiki-verify-multi` / `wiki-extract-concepts` / `wiki-import` это
  ...
  находится шаг рассуждения LLM, которым владеет orchestrator (см.
  [контракт `prepare`/`apply`](#the-prepare--apply-contract-decision-17)).
...
- `wiki-config show <папка>` печатает, откуда приходит каждое эффективное значение
  ...
  `wiki-config report --open` рендерит весь каскад одной HTML-страницей.
