...
| **`.wiki/layout.yaml`** | **Грамматика layout**: ЧТО и КАК индексировать — `ignore`, `type_mapping`, `paths`, `ref_extraction`, `drift_rules`/`coverage_rules` | Нет (база — встроенный `layout`) | По-ключевая: **`ignore` → UNION**, **`type_mapping` → deep-MERGE**, **`paths`/`ref_extraction` → REPLACE** |
