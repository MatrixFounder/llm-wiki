...
| **Поверхность** | 19 CLI (`wiki-*`); **у 17 из них** есть и slash-команда `/wiki-*` — `wiki-graph` и `wiki-health` это read-only репортёры без обёртки-команды |
| **Контракт ввода-вывода** | stdin/аргументы на вход → однострочный JSON-конверт в stdout + код возврата, **на каждый выполнившийся вызов подкоманды** (две границы — см. раздел об интеграционной модели) |
...
> инструментов над обычной папкой markdown. 19 команд `wiki-*` покрывают полный цикл:
...
19 CLI — это не плоский список: каждая играет роль в цикле выше. Ниже
...
| **`wiki-extract-decisions`** | *Типизированно-знаниевый* вход (TASK 063 / RFC-004): для **саммаризированной** заметки-источника извлекает **решения / требования / риски**, которые та фиксирует, — плюс типизированные **прямые рёбра** между ними (`implements` / `supersedes` / `causes` / …) — как полноценные страницы графа, которые `wiki-graph` может обходить, а `wiki-health` — проверять.<br>• Двухпроходный навык `prepare`/`apply`. `prepare` отдаёт **онтологический контракт** (реестр классов, домен/диапазон каждого ребра, enum `status` каждого класса) и ПРЕФЛАЙТИТ layout (G4 — отказывает *заранее*, если vault не отображает типизированные классы или целевая папка невидима для read-глобов layout: невидимая для глоба страница пишется, но не индексируется и не даёт ни одного lint-сигнала); `apply` проверяет **каждого** кандидата против этого контракта *до* первой записи (любое нарушение ⇒ exit 4, **ноль файлов**).<br>• **Анти-фабрикация — это механизм, а не просьба:** пустое извлечение — **УСПЕХ** (`no_candidates`, exit 0 — заметка без решений это нормальная заметка), а каждый `source_quote` обязан быть **дословным** из заметки; лазейки нет.<br>• Конфигурируется каскадным блоком `extract_decisions:` в `.wiki/sync.yaml`; `wiki-sync` / `wiki-import` отдают **маркер диспетчеризации** (никогда не inline-вызов LLM). ⚠️ Типизированные классы зависят от layout — `cybos` их отображает, `obsidian-personal` — **нет** (там `prepare` отказывает на G4). |
...
# извлечь типизированное знание (решения/требования/риски — layout обязан их отображать)
...
> - **живое выделение в редакторе (TASK 068)**: *«отредактируй выделенный текст»* →
...
- **Выходные данные** — это ровно **одна строка JSON в stdout** (`ensure_ascii=False`),
  *конверт*, **на каждый выполнившийся вызов подкоманды**. Конверты успеха несут поля
  результата; конверты сбоя несут ключ `"error"` со стабильным машиночитаемым кодом.
...
|---|---|
| `0` | Успех. |
| `1` | Обычно **необработанное исключение** — конверта нет, сырой traceback в stderr. ⚠️ **Три измеренных расхождения**: `wiki-lint --strict` (сработавший гейт, конверт *успеха*); `wiki-init` (`MISSING_VAULT_ARG` — конверт *ошибки* с кодом 1, по её собственной таблице в SKILL.md); `wiki-import` (`EXIT_USAGE = 1` — значение по умолчанию у `ImportArticleError`, так что, например, `FETCH_FAILED` тоже приходит сюда с конвертом ошибки). **Никогда не выводите «упало» из `$? == 1`** — проверьте, распарсился ли stdout. |
| `2` | Ошибка pipeline/предусловия (например, `wiki-query` `NO_CONTEXT`, `QUESTION_CHANGED`) — **и собственный код argparse** при отказе по использованию. |
...
### Две границы контракта конвертов

Утверждение выше — «один конверт **на каждый выполнившийся вызов подкоманды**», и уточнение
несущее: вызывающий, делающий `json.loads(stdout)` безусловно, ломается на обеих границах
(DF-072-3, измерено 2026-08-10 по всем 23 исполняемым файлам `bin/`):

1. **Отказ argparse предшествует контракту.** Нераспознанный флаг или отсутствующая
   подкоманда заставляют argparse напечатать usage в **stderr** и выйти с кодом **2**
   *до* того, как отработает код конверта — **stdout пуст, 22 из 22 CLI**. Показывайте текст
   из stderr; не парсите stdout. ✅ **Закреплено** тестом
   `tests/test_cli_envelope_contract.py`, который реально запускает каждую CLI из `bin/`.
2. **Три поверхности намеренно печатают не-JSON при успехе.** `wiki-search … --format
   markdown` (markdown), `wiki-sync scan … --dry-run` (человекочитаемый отчёт) и
   `wiki-config serve` (баннер с токенизированным URL в **stderr**, выход 0, **конверта в
   stdout нет вовсе** — чтобы токен не попал в машиночитаемый конверт; учтите, что stderr
   всё равно виден всему, что его захватывает: это большинство CI-логов и агент-обвязок).
   Это функции; выбирайте JSON-путь (`--format json`, `wiki-sync scan` без `--dry-run`),
   когда нужен парсинг. ⚠️ **Задокументировано, но НЕ закреплено тестом.**

Граница 1 — это измерение; граница 2 — поддерживаемый вручную список. Утверждать иначе
значило бы повторить тот самый дефект, ради которого написан этот раздел.
...
каждом инструменте. Золотое правило для интегрирующего агента: **сначала читайте
конверт и ветвитесь по его ключу `error`; `$?` используйте лишь как грубый
предфильтр — и никогда не выскребайте человеческую прозу.** Живые CLI делают
более сильную формулировку («ветвитесь по `$?`») прямо неверной: у `wiki-verify-multi`
код 6 — это *либо* записанный вердикт FAIL (конверт успеха), *либо* `INVALID_INDEX_DB`
(конверт ошибки, ничего не проверено); у `wiki-lint` код 1 — это *либо* сработавший
гейт `--strict` (конверт успеха), *либо* падение (конверта **нет** — различает пустой stdout);
а ещё код 1 несёт обычный конверт **ошибки** в `wiki-init` и `wiki-import` (см. строку `1` выше).
...
1. **`wiki-query prepare "<question>" --vault <vid> --vault-root <path> [scope…]`**
   ...
   - Exit 2 `NO_CONTEXT` → у vault нет основания. **Остановитесь; не синтезируйте
     извне vault** (защита от галлюцинаций). ⚠️ Повтор с `--min-hits 0` не поможет:
     этот флаг лишь отключает ДАННЫЙ отказ и не наполняет retrieval. **Здесь retrieval
     пуст**, поэтому `apply` отклонит и пустой массив цитирований (`NO_CITATIONS`), и
     непустой (`CITATION_NOT_RETRIEVED`). «Источники не найдены» сообщают оператору, а
     не записывают в vault.
...
3. **`wiki-query apply … --question-hash <verbatim from prepare> --answer-stdin
   ...
   молча). **Пустой** массив цитирований → exit 4 `NO_CITATIONS`: ответ обязан
   цитировать ≥1 извлечённый источник. Не добивайте массив, чтобы пройти проверку —
   либо цитируйте реальные `hits`, либо остановитесь и сообщите о пробеле.
...
**Сами контракты-навыки защищены по целостности (H-5, TASK 067).** H-6 защищает от
враждебного *источника*; H-5 защищает *сами промпты* рассуждения. Файлы `SKILL.md` +
`references/*.md`, которые загружаются **дословно** в контекст агента как
контракты рассуждения/безопасности — извлечения, синтеза, верификации и
safety-tier-контракт `obsidian-cli` — закреплены по SHA-256 в
`config/skill-integrity.sha256`. `prepare` каждого рельса проверяет закреплённый
хэш своего контракта и сообщает о дрейфе в блоке envelope **`integrity`**; workflow
ОСТАНАВЛИВАЕТСЯ перед загрузкой дрейфующего контракта, а
`WIKI_STRICT_SKILL_INTEGRITY=1` заставляет `prepare` отказать (exit 2). Гейт
`tests/test_h5_skill_integrity.py` краснеет на любой не-перезакреплённой правке — так
что подмена загружаемого промпта это **обозримый diff манифеста и падающая сборка**,
а не тихая supply-chain-правка. Перезакрепить *одобренную* правку:
`python3 scripts/pin_skill_integrity.py --write`.

> **✅ Главное из «Wiki как внешнего ресурса».** Контракт для любого агента: одна
> строка JSON в stdout + стабильный код выхода **на каждый выполнившийся вызов
> подкоманды** — **читайте `.error` из конверта и ветвитесь по нему**, а `$?` берите
> лишь как грубый предфильтр; прозу не выскребайте. (Отказ по использованию
> предшествует контракту: stderr, код 2, пустой stdout. Код 6 у `wiki-verify-multi`
> и код 1 у `wiki-lint` несут по два значения, поэтому один только `$?` не отличит
> записанный вердикт от отказа конфига, а сработавший гейт — от падения.) Команды LLM-формы разрезаны на `prepare`/`apply`, шаг
...
> контент — данные, а не инструкции (H-6); а загружаемые контракты рассуждения
> закреплены по хэшу (H-5), так что `prepare` с `integrity.status != "ok"` означает STOP.
...
- `external` — у страницы есть **`http(s)`-ключ происхождения** во frontmatter:
  `source:` / `url:` / `URL:` **и их регистровые варианты** `Source:` / `SOURCE:` /
  `Url:` (TASK 061; `wiki-import` пишет верхнерегистровый `URL:`, веб-клипперы часто
  пишут `Source:`). Это захваты/вырезки веб-страниц, которые могут нести встроенные
  инструкции (H-6, косвенная prompt-инъекция). *Страница под `_raw/` тоже считается
  external, но на практике вы с этим не столкнётесь: все встроенные layout'ы исключают
  `**/_raw/**` из индекса, поэтому capture из `_raw/` никогда не попадает в выдачу —
  рабочий сигнал именно ключ во frontmatter.*
...
| Выскребать прозу CLI вместо JSON-конверта / кода выхода | Конверт — это контракт; проза — нет. Читайте `.error` из конверта и ветвитесь по нему; один `$?` — это грубый предфильтр (коды 6 у `wiki-verify-multi` и 1 у `wiki-lint` означают по два разных исхода). |
| Безусловный `json.loads(stdout)` | Отказ по использованию вообще не доходит до кода конверта: argparse пишет в **stderr**, выходит с кодом 2 и оставляет stdout **пустым** — парсинг падает вместо того, чтобы показать оператору нужное сообщение. Сначала проверьте код выхода и непустой stdout (DF-072-3). |
...
- **JSON-конверт** — единственная строка JSON, которую команда печатает в stdout как
  машиночитаемый результат **на каждый выполнившийся вызов подкоманды**. Отказ argparse
  происходит *до* конверта (usage в stderr, код 2, пустой stdout), а `--format markdown`,
  `wiki-sync scan --dry-run` и `wiki-config serve` намеренно печатают не-JSON.
- **Код выхода** — числовой результат процесса (`$?`); грубый предфильтр для
  интегрирующего агента. Первичный сигнал ветвления — ключ `error` в конверте.
...
- **Trust (`--min-trust`)** — производный уровень происхождения хита:
  `external` (во frontmatter есть `http(s)`-ключ `source:`/`url:`/`URL:` + регистровые
  варианты — веб-захват или вырезка; `_raw/` это backstop, который ни один встроенный
  layout не индексирует), `internal` (ваша заметка), `verified` (за неё ручается
  verification-страница); нижняя граница для заземления ответа.
