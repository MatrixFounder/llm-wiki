...
| **R-4** | D2a provenance NFC/NFD normalisation (dogfood #3; folded into TASK 024) | ✅ | (a) `_resummarize.summary_exists` NFC-normalises BOTH the D2a citation set and the target before equality — a Cyrillic-named raw whose FS-walked `rel` is **NFD** (macOS) must match its summary's **NFC** `sources:`; (b) localised to D2a (D1 source_state is NFD-on-both-sides self-consistent, D2b mirror is FS-vs-FS); (c) regression `test_gate_d2a_provenance_nfc_nfd` (NFD `rel` ↔ NFC `sources:` → skip:provenance) + dogfood reconfirm (`Кейс Ярли …pptx` → skip, zero convert+ingest); zero DDL, no new deps |
