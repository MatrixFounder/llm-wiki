...
```jsonc
{
  "title_ru":  "string",            // RU title
  "title_orig":"string?",           // original-language title
  "author":    "string|null",
  "published": "YYYY-MM-DD|null",
  "tldr":      "string",            // 1–2 RU sentences
  "summary_bullets": ["string", …], // RU key points / conclusions
  "ru_body":   "string|null",       // see depth-by-mode
...
## Depth by mode
| mode | `ru_body` | `summary_bullets` | use for |
|---|---|---|---|
| `full` | **complete** fluent RU translation (preserve headings/lists/tables; keep code/formulae/tickers; translate everything) | 4–7 takeaways | digestible web articles, Wikipedia, Investopedia |
...
| `thread` | tight RU конспект, 2–5 paragraphs (distil the argument; drop reply-counts/handles/metrics) | 3–6 core claims | X/Twitter threads (attribute as one author's opinion) |

If the source is already RU (`prepare` engine on a RU page), `ru_body` is the *cleaned*
RU body (strip conversion artifacts), not a re-translation.
