...
| **6** | `VAULT_NOT_FOUND` | `--vault <id>` names a vault that is **not registered in this index**. Checked before any check runs; nothing is linted. ⚠️ Until DF-074-4 this returned **`{"total_issues": 0}` at exit 0** — a clean bill of health for a vault that does not exist, on the surface this project documents as its CI gate. Omitting `--vault` (all-vaults mode) is unaffected. |
